...
构建镜像前先在仓库根目录生成 arm64 Linux 版本的 Rust 面板二进制：
...
交叉编译需要系统 linker。脚本默认检查 `aarch64-linux-gnu-gcc`，也可以通过 `LINUX_LINKER` 指定兼容 linker。
...
./DepotDownloader -app 343050 -os linux -osarch 64 -dir /data/dst-dedicated-server -validate
```
