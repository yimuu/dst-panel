...
用于构建 DST Admin Rust 的标准 Docker 镜像（Linux x86_64 架构）。
...
容器内重要路径说明。入口脚本会在首次启动时把镜像内置默认配置复制到 `/data/config.yml`，并将应用配置改为 `dataDir: "."`；随后服务会从 `/data` 工作目录直接读写数据库、日志、静态资源和 DST 运行文件。

| 容器内路径 | 用途 | 是否推荐挂载 |
|-----------|------|-------------|
| `/data` | 统一持久化数据卷 | ✅ 推荐 |
| `/data/config.yml` | 应用运行配置，默认 `dataDir: "."` | 自动 |
| `/data/dst_config` | DST 安装和集群默认配置 | 自动 |
| `/data/klei` | 游戏存档目录，入口脚本映射给 Klei 默认路径 | 自动 |
| `/data/backup` | 存档备份目录 | 自动 |
| `/data/mod` | MOD 缓存目录 | 自动 |
| `/data/steamcmd` | SteamCMD 安装目录 | 自动 |
| `/data/dst-dedicated-server` | 饥荒服务器文件 | 自动 |
| `/data/dst-db` | SQLite 数据库文件 | 自动 |
| `/data/password.txt` | 初始密码文件 | 自动 |
| `/data/first` | 首次登录标记文件 | 自动 |
| `/data/dst-admin-go.log` | 应用日志文件，保留兼容文件名 | 自动 |
...
services:
  dst-admin:
    ...
    volumes:
      ...
      # 统一持久化数据卷，入口脚本和应用会直接读写 /data 下的运行文件
      - ${PWD}/dstsave:/data
    environment:
      - TZ=Asia/Shanghai
```
...
# 查看应用日志
docker exec -it dst-admin cat /data/dst-admin-go.log
```
